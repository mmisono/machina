...
def mp_sample(pol, env, max_steps, max_epis, n_steps_global, n_epis_global,
              epis, exec_flag, deterministic_flag, process_id, prepro=None, seed=256):
    """
    Multiprocess sample.
...
class EpiSampler(object):
    ...
    def __init__(self, env, pol, plasma_socket_name="/tmp/plasma",
                 num_parallel=8, prepro=None, compress=False, seed=256):
        self.env = env
        ...
        self.client = plasma.connect(plasma_socket_name)
        ...
        for ind in range(self.num_parallel):
            p = mp.Process(target=mp_sample, args=(self.pol, env, self.max_steps, self.max_epis, self.n_steps_global,
                                                   self.n_epis_global, self.epis,
                                                   self.exec_flags[ind], self.deterministic_flag, ind, prepro, seed))
            p.start()
    ...
    def sample(self, pol, max_epis=None, max_steps=None, deterministic=False):
        ...
        while True:
            if all([exec_flag == 0 for exec_flag in self.exec_flags]):
                epis = list(self.epis)
                if self.compress:
                    epis = pa.serialize(epis).to_buffer().to_pybytes()
                    epis = lz4.frame.compress(epis)
                self.object_id = self.client.put(epis)
