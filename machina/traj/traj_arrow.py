...
class Traj(object):
    ...
    Parameters
    ----------
    object_id: plasam.ObjectId
        Object ID of the epi data
    ...
    def __init__(self, object_id, plasma_socket_name="/tmp/plasma",
                 max_steps=None, traj_device=None, ddp=False, compress=False):
        self.data_map = dict()
        self._next_id = 0
        ...
        client = plasma.connect(plasma_socket_name)
        epis = client.get(object_id)
        if compress:
            epis = lz4.frame.decompress(epis)
            epis = pa.deserialize(epis)
        self.add_epis(epis)
        client.disconnect()

        if traj_device is None:
            self.traj_device = get_device()
        else:
    ...
    def pin_memory(self):
        for key in self.data_map:
            self.data_map[key] = self.data_map[key].pin_memory()

    def make_shared(self):
        for key in self.data_map:
            self.data_map[key].share_memory_()
    ...
    def _get_indices(self, indices=None, shuffle=True):
        if indices is None:
            if shuffle:
                indices = torch.randperm(self.num_step, dtype=torch.long)
            else:
                indices = torch.arange(self.num_step, dtype=torch.long)
        elif shuffle:
    ...
    def iterate(self, batch_size, epoch=1, indices=None, shuffle=True):
        ...
        for _ in range(epoch):
            indices_ = self._get_indices(indices, shuffle)
            while self._next_id <= len(indices_) - batch_size:
                yield self._next_batch(batch_size, indices_)
            self._next_id = 0
